...
const Nav = ({ activeView, setActiveView }) => {
    // Definitive list of unique navigation items
    const navItems = [
        ...
        { id: 'stock', label: 'Stock', icon: React.createElement('svg', { xmlns: "http://www.w3.org/2000/svg", className: "h-5 w-5 sm:mr-2", viewBox: "0 0 20 20", fill: "currentColor" }, React.createElement('path', { fillRule: "evenodd", d: "M5 2a1 1 0 00-1 1v1H3a1 1 0 00-1 1v12a1 1 0 001 1h14a1 1 0 001-1V5a1 1 0 00-1-1h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z", clipRule: "evenodd" })) },
        ...
        React.createElement('button',
            {
                ...
                className: `flex-1 flex flex-col sm:flex-row items-center justify-center text-[10px] sm:text-base font-medium py-2 sm:py-3 px-1 sm:px-5 sm:border-b-2 transition-colors duration-200 ${
                    activeView === item.id
            ...
            item.icon,
            React.createElement('span', { className: "whitespace-nowrap mt-0.5 sm:mt-0" }, item.label)
        )
    ));
    
    // Don't show navigation bar on the home screen
    if (activeView === 'home') {
        return null;
    }

    return (
        React.createElement('nav', { className: "sm:bg-white sm:dark:bg-gray-800 sm:shadow-md sm:relative fixed bottom-0 left-0 right-0 z-20 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 sm:border-t-0 pb-safe-bottom" },
            React.createElement('div', { className: "w-full flex justify-around sm:hidden" },
