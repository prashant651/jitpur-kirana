...
const TableHeader = () => (
    React.createElement('thead', { className: "bg-gray-100 dark:bg-gray-700" },
        React.createElement('tr', null,
            React.createElement('th', { scope: "col", className: "px-2 py-3 text-left text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[40%] min-w-[130px]" }, "Item Name"),
            React.createElement('th', { scope: "col", className: "px-1 py-3 text-center text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[12%]" }, "Open"),
            React.createElement('th', { scope: "col", className: "px-1 py-3 text-center text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[12%]" }, "Close"),
            React.createElement('th', { scope: "col", className: "px-1 py-3 text-center text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[10%]" }, "Sold"),
            React.createElement('th', { scope: "col", className: "px-1 py-3 text-center text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[12%]" }, "Rate"),
            React.createElement('th', { scope: "col", className: "px-2 py-3 text-right text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider w-[14%]" }, "Total"),
            React.createElement('th', { scope: "col", className: "w-[5%] px-1" }, React.createElement('span', { className: "sr-only" }, "Actions"))
        )
    )
);
...
const TableRow = ({ item, onUpdate, onRemove, isReadOnly }) => {
    ...
    // min-w-0 is critical to prevent inputs from stretching table columns in Safari
    const inputClass = "w-full min-w-0 bg-transparent p-1 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 disabled:text-gray-500 dark:disabled:text-gray-400 text-[11px] sm:text-sm border-none shadow-none";

    return (
        React.createElement('tr', { className: "bg-white dark:bg-gray-800 even:bg-gray-50 dark:even:bg-gray-800/50" },
            React.createElement('td', { className: "px-2 py-2 truncate" }, 
            ...
            React.createElement('td', { className: "px-1 py-2 text-center text-[11px] sm:text-sm font-bold text-gray-900 dark:text-gray-100" }, 
                soldQty
            ),
            React.createElement('td', { className: "px-1 py-2 text-center" }, 
            ...
            React.createElement('td', { className: "px-2 py-2 text-right text-[11px] sm:text-sm font-bold text-gray-900 dark:text-gray-100" }, 
                totalAmount.toLocaleString('en-IN')
            ),
            React.createElement('td', { className: "px-1 py-2 text-center" },
                !isReadOnly && (
                    React.createElement('button', { onClick: onRemove, className: "text-red-500 hover:text-red-700 transition-colors", "aria-label": "Remove item" },
    ...
    return (
         React.createElement('div', { className: "bg-white dark:bg-gray-800 p-2 sm:p-4 rounded-lg shadow border border-gray-200 dark:border-gray-700" },
            React.createElement('div', { className: "flex justify-between items-center mb-3 px-1" },
                 ...
                 !isReadOnly && (
                     React.createElement('button', { onClick: handleAddItem, className: "px-3 py-1 bg-green-600 text-white font-bold rounded-md hover:bg-green-700 transition-colors text-[10px] uppercase tracking-wide shadow-sm" },
                        "+ Add Item"
                     )
                 )
            ),
            React.createElement('div', { className: "overflow-x-auto -mx-2 sm:mx-0" },
                React.createElement('table', { className: "min-w-full table-fixed divide-y divide-gray-200 dark:divide-gray-700" },
